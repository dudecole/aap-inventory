...
baseurl=$baserepo/$(grep tower_package_version "${GROUP_VARS_ALL}" | grep -o '[[:digit:]].[[:digit:]]')/epel-$1-x86_64
enabled=0
gpgcheck=$GPGCHECK
            ...
            if [ $? -eq 0 ]; then
                ...
                if [ -z "$AW_REPO_URL" ]; then
                    if [ "$(distribution_id)" == "rhel" ]; then
                        aap_xy_version=$(grep 'automation_platform_version:' "${GROUP_VARS_ALL}" |cut -d \' -f 2 |cut -d \. -f 1,2)
                        aap_channel="ansible-automation-platform-${aap_xy_version}-for-rhel-${DISTRIBUTION_MAJOR_VERSION}-x86_64-rpms"
